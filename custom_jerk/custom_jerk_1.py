...
DEFAULT_EXPLORE_STEP = 100
DEFAULT_BACKTRACK_STEP = 70
    ...
    while True:
        if new_ep:
            if (solutions and
                ...
                env.reset()
                new_ep = False
        rew, new_ep, max_rew_step = move(env, DEFAULT_EXPLORE_STEP)
        if not new_ep and rew <= 0:
            print('backtracking due to negative reward: %f' % rew)
            _, new_ep, _ = move(env, DEFAULT_EXPLORE_STEP - max_rew_step, left=True)
        if new_ep:
...
def move(env, num_steps, left=False, jump_prob=1.0 / 10.0, jump_repeat=4):
    ...
    while not done and steps_taken < num_steps:
        ...
        if total_rew >= max_rew:
            max_rew = total_rew
            max_rew_step = steps_taken
        steps_taken += 1
        if done:
            break
    return total_rew, done, max_rew_step
