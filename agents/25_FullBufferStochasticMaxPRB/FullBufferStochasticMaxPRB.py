...
class FullBufferStochasticMaxPRB(PrioritizedReplayBuffer):
    """
    ...
    def add_sample(self, sample, init_weight=None):
        ...
        if init_weight is None:
            ...
            if len(self.transitions) < self.capacity: # If buffer is not full, always add
                self.transitions.append(sample)
                self.errors.append(new_error)                
            elif random.random() < new_error / self.errors.max(): # If buffer is full, add stochastically
                self.transitions.append(sample)
                self.errors.append(new_error)
